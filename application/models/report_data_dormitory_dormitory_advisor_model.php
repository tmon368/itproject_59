...
class report_data_dormitory_dormitory_advisor_model extends CI_Model {
 ...
 public function showAll(){
   ...
   $this->db->where('dm.dorm_ID',7);
  // $this->db->where('oc.oc_ID',$oc_ID);
   $this->db->order_by('dm.dorm_ID');
   ...
   foreach ($data as $key=>$value){
      ...
      if ($value['dorm_ID'] != $chk_dormID){
         $count = 1;
         $chk_dormID = $value['dorm_ID'];
