...
class Discipline_officer_dashboard extends CI_Controller {
	...
	public function update()
	{
		...
		public function showAll(){
			$result = $this->Discipline_officer_dashboard_model->showAll();
			echo json_encode($result);
		}
		...
		public function addnotify(){
			$result = $this->Discipline_officer_dashboard_model->addnotify();
			//$msg['success'] = false;
			//$msg['type'] = 'add';
			/*
		...
		public function editnotify(){
			...
			$result = $this->Discipline_officer_dashboard_model->editnotify();
			echo json_encode($result);
		}
		
		//ฟังก์ชันการอัพเดตข้อมูล เมื่ออัพเดตข้อมูลเสร็จสิ้นจะแสดงข้อความ แก้ไขข้อมูลเรียบร้อย
		public function updatenotify(){
			$result = $this->Discipline_officer_dashboard_model->updatenotify();
			$msg['success'] = false;
			$msg['type'] = 'update';
			if($result){
				$msg['success'] = true;
		...
		public function deletenotify(){
			$result = $this->Discipline_officer_dashboard_model->deletenotify();
			/*
			if($result){
				$msg['success'] = true;     
			}else{
		...
		function selectstudent(){
			$result =  $this->Discipline_officer_dashboard_model->selectstudent();
			echo json_encode($result);
		}
		...
		function selectplace(){
			...
			$result = $this->Discipline_officer_dashboard_model->selectplace();
			echo json_encode($result);
		}
		function selectvehicles(){
		   
			
			$result = $this->Discipline_officer_dashboard_model->selectvehicles();
		...
		function selectoffevidence(){
			...
			$result = $this->Discipline_officer_dashboard_model->selectoffevidence();
		...
		function selectoffensecate(){
			...
			$result = $this->Discipline_officer_dashboard_model->selectoffensecate();
		...
		function selectOffenseoffevidence(){
			...
			$result = $this->Discipline_officer_dashboard_model->selectOffenseoffevidence();
			//print_r($result);
			echo json_encode($result);
		}
	
		function spc_showoffhead(){
			$result = $this->Discipline_officer_dashboard_model->spc_showoffhead();
			echo json_encode($result);
		}
	
		function check_id (){
			//เช็คค่า id ที่มากสุด
			$result = $this->Discipline_officer_dashboard_model->check_id();
			echo json_encode($result);
		...
		function selectplaceall (){
			//เช็คค่า id ที่มากสุด
			$result = $this->Discipline_officer_dashboard_model->selectplaceall();
			echo json_encode($result);
			//ยังไม่ได้เขียน model
		}
		function selectregist_num (){
			$result = $this->Discipline_officer_dashboard_model->selectregist_num();
			echo json_encode($result);
		}
	}
